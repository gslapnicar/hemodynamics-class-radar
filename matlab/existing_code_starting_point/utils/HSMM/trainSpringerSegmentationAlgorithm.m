...
function [logistic_regression_B_matrix, pi_vector, total_obs_distribution] = trainSpringerSegmentationAlgorithm(PCGCellArray, annotationsArray, FsAllData, figures)
...
%     [PCG_Features, featuresFs] = getSpringerPCGFeatures(PCG_audio, Fs);
    PCG_Features = getSpringerPCGFeaturesNoResampling(PCG_audio, Fs);
    
    resampFactor = featuresFs/Fs;
    S1_locations = round(S1_locations.*resampFactor); % downsample to featuresFs
    S2_locations = round(S2_locations.*resampFactor); % downsample to featuresFs
