...
fc_high_Schmidt = springer_options_advanced.fc_high_Schmidt;
fc_low_Schmidt = springer_options_advanced.fc_low_Schmidt;
if fc_high_Schmidt > 249
    if Fs < 2000
        fc_high_Schmidt = 249;
    end
end
if new_options.HSMM_FILTER_TWICE
    if fc_high_Schmidt ~= 0 && fc_low_Schmidt ~= 0
        audio_data = butterworth_low_pass_filter(audio_data,2,fc_high_Schmidt,Fs, false); % Edited (Kilin)
        audio_data = butterworth_high_pass_filter(audio_data,2,fc_low_Schmidt,Fs); % Edited (Kilin)
    end
end
...
% y=audio_data-mean(audio_data); % edited (Kilin) 31.03.2017
[c] = xcorr(y,'coeff'); % 'coeff' scales signal so that amp. at lag zero is 1
signal_autocorrelation = c(length(homomorphic_envelope)+1:end);

min_index = springer_options_advanced.min_index*Fs;
max_index = springer_options_advanced.max_index*Fs;
if max_index > length(signal_autocorrelation)
    max_index = length(signal_autocorrelation);
end
