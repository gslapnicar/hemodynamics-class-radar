...
%% Use Sphygmogram features
% if new_options.USE_SPHYG_FEATURE
%     data_temp = diff(audio_data).^2;
%     downsampled_date_temp = resample(data_temp,featuresFs, Fs);
%     PCG_Features = downsampled_date_temp;
%     return
% end
...
if new_options.HSMM_FILTER_TWICE
    if fc_high_getSpringer ~= 0 && fc_low_getSpringer ~= 0
        audio_data = butterworth_low_pass_filter(audio_data,2,fc_high_getSpringer,Fs, figures);
        audio_data = butterworth_high_pass_filter(audio_data,4,fc_low_getSpringer,Fs, figures);
    end
end

%% Spike removal from the original paper:
audio_data = schmidt_spike_removal(audio_data,Fs); %edited Kilin 21.03.2017
